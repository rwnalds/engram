 ...
 *
 *   bun scripts/mcp-stdio.ts [vault-dir]
 *
 * Defaults to the bundled sample-vault when no dir is given. VAULT_DIR env also works.
 * The vault dir MUST be resolved into env BEFORE the tools module loads (config reads it at
 * import time), so the tool imports are dynamic and happen after this block.
 */
