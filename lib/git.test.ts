...
async function seedRemoteAndClone() {
  ...
  await seed.push(["-u", "origin", "HEAD:refs/heads/main"]);
  // `git init --bare` may point HEAD at `master`; without this the vault clone lands detached
  // and every push fails for a reason that has nothing to do with what these tests measure.
  await simpleGit(REMOTE).raw(["symbolic-ref", "HEAD", "refs/heads/main"]);
  await seed.raw(["branch", "--set-upstream-to=origin/main"]).catch(() => {});

  await simpleGit().clone(REMOTE, VAULT);
  const v = vaultGit();
  await v.addConfig("user.email", "engram@test");
  await v.addConfig("user.name", "Engram");
  await v.checkout(["-B", "main", "--track", "origin/main"]).catch(() => {});

  fs.writeFileSync(
    path.join(process.env.ENGRAM_DATA_DIR!, "repos.json"),
    JSON.stringify([
      { id: REPO_ID, name: "vault", url: REMOTE, branch: "main", active: true, addedAt: "2026-01-01T00:00:00.000Z" },
    ]),
  );
}
